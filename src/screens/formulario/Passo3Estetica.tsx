...
import { PassoProps, ChecklistEstetica, ItemChecklist } from '../../types';
import ProgressoPasso from '../../components/ProgressoPasso';

const ITENS: { chave: keyof ChecklistEstetica; label: string; descricao: string }[] = [
  { chave: 'lataria',      label: 'Lataria',      descricao: 'Amassados, arranhões, ferrugem ou retoques de pintura' },
  { chave: 'vidros',       label: 'Vidros',        descricao: 'Trincas, quebras ou manchas nos vidros laterais e traseiro' },
  { chave: 'farois',       label: 'Faróis',        descricao: 'Dianteiros e lanternas traseiras — trincas ou umidade interna' },
  { chave: 'para_brisas',  label: 'Para-brisa',    descricao: 'Trincas, pedradas ou danos que comprometam a visibilidade' },
  { chave: 'para_choques', label: 'Para-choques',  descricao: 'Dianteiro e traseiro — rachaduras, amassados ou peças soltas' },
];
...
export default function Passo3Estetica({ dados, atualizar, onNext, onBack, passo, totalPassos }: PassoProps) {
  const checklist = dados.checklist_estetica;

  function setStatus(chave: keyof ChecklistEstetica, novoStatus: ItemChecklist['status']) {
    const atual = checklist[chave];
    const novo: ItemChecklist = novoStatus === 'avaria'
      ? { status: 'avaria', foto: atual.foto }
      : { status: novoStatus };
    atualizar({ checklist_estetica: { ...checklist, [chave]: novo } });
  }

  function setFoto(chave: keyof ChecklistEstetica, uri: string) {
    atualizar({ checklist_estetica: { ...checklist, [chave]: { ...checklist[chave], foto: uri } } });
  }
  ...
  return (
    <View style={styles.container}>
      <ProgressoPasso passo={passo} totalPassos={totalPassos} titulo="Checklist Estética" />
